...
class Review(object):
    ...
    def get_aos(self) -> List[List[AspectOpinionSentiment]]:
        ...
        for i, aos in enumerate(self.aos):
            if self.implicit[i]:
                r.append([([None], [self.sentences[i][j] for j in o], s) for aos_tuple in aos for (a, o, s) in [aos_tuple[:3]]])
            else:
                r.append([([self.sentences[i][j] for j in a], [self.sentences[i][j] for j in o], s) for aos_tuple in aos for (a, o, s) in [aos_tuple[:3]]])
        return r
